...
export class ProfileEditComponent implements OnInit {
  ...
  onEditSubmit(): any {
    ...
    if (this.password1 !== this.password2) {
      Swal.fire({
        title: '실패',
        inputLabel: '패스워드가 일치하지 않습니다. 다시 입력하세요.',
        icon: 'error',
    ...
    if (this.password1.length < 8 || this.password1.length > 20) {
      Swal.fire({
        title: '실패',
        inputLabel: '비밀번호를 8자리 ~ 20자리 이내로 입력해주세요.',
        icon: 'error',
        confirmButtonText: '다시시도',
        timer: 1500,
      });
